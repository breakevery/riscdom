...
1. **消除 QEMU 的 TCP 端口依赖**：QMP 走 stdio（配置开关后）、串口走 file 变体，然后退役那些重试 ——
   [docs/qemu-stdio.md](docs/qemu-stdio.md) §6。
2. **macOS / Linux 支持与多 OS CI 矩阵**。
3. **多 VM 并行**：同一时间驱动多台宿主持有的 guest。
4. **增量快照 + 加密**。
5. **会话加密 / 导出 / 搜索**。
6. **多 AI 社会与 `Governance` trait**（宪法中的实验变量）。
7. **双语界面**：界面文案目前硬编码在各组件里（应用内是中文，面板文档是英文），没有语言开关。把界面
   做成可翻译是一个独立特性。
