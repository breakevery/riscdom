...
1. **消除 QEMU 的 TCP 端口依赖，与统一的 relay 端口租约**：relay 端口由单一租约统一分配，进而让 QMP
   （与串口）彻底不再需要 TCP 端口（提案见 [docs/qemu-stdio.md](docs/qemu-stdio.md)）。**舍弃「方案 3」
   这个提法**：本仓库没有任何地方记录过那份选项清单，读者无从查证。
...
11. **[DONE] 主题切换**：浅色 / 深色 / 跟随系统，在外观页循环切换，由 `ui/src/lib/theme.ts` 统一应用
...
1. **双语界面**：界面文案目前硬编码在各组件里（应用内是中文，面板文档是英文），没有语言开关。把界面
   做成可翻译是一个独立特性，不属于 v0.4 路线图。
