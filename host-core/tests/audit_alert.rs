...
use host_core::events::{RecordingEventSink, EV_AUDIT_FAILED};
...
fn the_failure_queue_stays_bounded() {
    ...
    for i in 0..(host_core::state::AUDIT_FAILURE_QUEUE_CAP + 5) {
        state.push_audit_failure(&format!("failure {i}"));
    }
    let failures = state.audit_failures();
    assert_eq!(failures.len(), host_core::state::AUDIT_FAILURE_QUEUE_CAP);
    // The newest ones survive: they describe the current state.
    let expected = format!("failure {}", host_core::state::AUDIT_FAILURE_QUEUE_CAP + 4);
