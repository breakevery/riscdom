...
use host_core::state::{AgentOutcomeView, AppState};
...
pub fn tool_steps(events: &[host_core::StoredEventView]) -> Vec<ToolStep> {
    let mut ordered: Vec<&host_core::StoredEventView> = events.iter().collect();
...
pub fn report(
    ...
    let counts = [
        host_core::events::EV_AGENT_ITERATION,
        host_core::events::EV_AGENT_TOOL_CALL,
        host_core::events::EV_AGENT_TOOL_RESULT,
        host_core::events::EV_AGENT_FINAL,
        host_core::events::EV_SERIAL_CHUNK,
        host_core::events::EV_VM_STATE,
        host_core::events::EV_PREFLIGHT,
    ]
    .iter()
