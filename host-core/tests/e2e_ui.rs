...
//!
//! NOTE: the task listed this as `agent/tests/e2e_ui.rs`, but it exercises the
//! host's `AppState::run_agent` path + host events. Putting it under
//! `agent/tests` would require an `agent → host-core` dev-dependency, i.e. a
//! dependency cycle (host-core already depends on agent). It therefore lives in
//! `host-core/tests`.
//!
//! Manual run:
//!
...
use host_core::events::{RecordingEventSink, EV_AGENT_FINAL, EV_AGENT_TOOL_CALL, EV_SERIAL_CHUNK};
use host_core::state::AppState;
use host_core::ChainStatusView;
...
fn e2e_ui_mock_run_emits_final_and_serial() {
    ...
    *state.llm_override.lock().unwrap() = Some(Arc::new(MockLlm::new(script)));
    ...
    let result = state.run_agent(
        sink.clone() as Arc<dyn host_core::EventSink>,
