...
- **网络接入的配置页**（v0.9.9 内网接入 第 2 批）：*设置 → 网络*把两个方向放在一处——连出去到内网服务器（地址与令牌；连接本身在下一批），以及把本节点的看板开放到网络（开关、绑定地址、以及一个一勾上就给出警示的允许局域网开关）。`settings.json` 增量新增 `network`，shell crate 新增 `get_network` / `set_network` / `read_lan_token`——最后一个**只读 token 文件，从不创建**——而浏览器一概拿不到。

## [0.9.1] - 2026-09-25

### 修复

- **桌面端停在登录页**（v0.9.1）：`App.tsx` 对所有运行时都要 token，而桌面端既没有 token、也没有可用来校验的 `/v0/health`，于是 v0.9.0 的桌面构建完全不可用。现在**先**回答桌面端，token 门住在一个它永不进入的组件里。
...
- **一个死掉的规格字段被删除，迁移 relay 现在预留自己的端口**（v0.9 发布前小修）。`DownloadSpec` 与 `QemuDownloadSpec` 不再携带 `install_subdir`——生产代码从不读它、只有一个测试断言过它，因此 §49 那句「继续是死的」已被取代（决策 §64）；而 `MigrationRelay::bind_local_with_timeout` 现在会在本进程的登记表里预留它的回环端口，并持有一个与 relay 同寿的 `PortLease`（决策 §65）——这正是 `lease_local_ports` 早已给出的保证，公开构造函数未变。§1 里那些仍写着「仍开着」的注记（gate 的测试覆盖、logging 批次的根因之问）现在都记下了是什么关闭了它们。行为未变。
