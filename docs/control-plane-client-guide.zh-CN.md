...
共 32 个，全部 `GET`。应答是宿主的视图类型，字段见 `host-core/src/state.rs`。一律 JSON。
...
桌面外壳可以被指向**另一个**节点，而不是它自己进程里的那个；它由此变成的东西，恰好就是上面描述的那个客户端：同一批界面、同一个适配层，只是把 Tauri IPC 换成 HTTP。在对着它写东西之前，有两件事值得知道：

- **模式在启动时定下。** 地址是 `settings.json` 里的一个偏好；token 是 **OS 钥匙串**里的凭据，账户名 `remote-token:<host>`——与 provider key 同一个家，因为机密不属于设置文件。改任何一项都是改*这个窗口跟哪个 host 说话*，所以它在应用再次启动时生效，而不是按键即生效。
- **远端窗口与浏览器一样是只读的**，并且多留一页浏览器没有的界面：*网络* tab——「断开并改用本机」就在那里。那个按钮（以及它后面的四个命令）在**任何模式下都作用于跑这个窗口的那台机器**：服务器不可达的客户端，仍必须能让自己不再做那个客户端。

逐步走查（含两进程配方）在 [manual-acceptance.zh-CN.md](manual-acceptance.zh-CN.md) 的层次 9。
