...
指纹，两次 run 也都会出现在 run 列表里。**缺的是可读性**：没有任何地方显示两次 run **差在哪些字段**，run
列表也不显示这次 run 是从哪个快照恢复的。
...
机器上照做。** 第 1–2 步是机器装配，在本仓库的 CI 里无法诚实自动化（runner 没有 QEMU、没有 GUI）；第
...
| **(b) 文档里的**人工清单，每个版本由人用真实 API key 走一遍 | **推荐，用于第 1–2 步** | 这是覆盖「装 QEMU」与「粘贴 key」的唯一诚实方式。代价：每版的人工时间，且必须被**记录下来**才算数 |
...
6. **发布门槛包含「由人用真实 API key 走一遍第 1–2 步」**，且必须**对照一份清单模板记录下来**：
   ...
   `audit-verify` 结论。没人写下来的行走，没人能复核。**这份模板属于 v0.5 工作，目前还不存在。**
