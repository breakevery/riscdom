...
| 5 | 得审计记录 | 一次已结束的 run | **导出它**（今天无按钮可按） | 一个第三方能用 `audit-verify` 验的文件 | 链已断 → 导出不得假装没断 |
...
- **导出什么**：闭区间 `[start_seq, end_seq]` 内的事件 —— 其中就包含那次 run 的 `run.start`（带着
  ...
  run 的元数据**本来就在**被导出的事件里，所以文件能自解释，并且仍可逐行验证。
- **什么格式**：`export_jsonl` 已经在写的那个 JSONL，原样不动 —— 每行 `id` / `timestamp_ms` / `actor` /
  `action` / `detail` / `prev_hash` / `hash`，按 id 序。读者无需本应用即可逐行重算哈希并检查链接。
- **导出到哪**：工作区下的一个默认路径，用原生**另存为**对话框让用户自选。（工作区正是导出命令被允许写入
  的范围；`.riscdom/` 会被文件列表跳过，所以默认放一个可见路径比放隐藏的内部目录更好。）
...
| 6 rollback | 已有 | 无 |
| 7 换配置重跑 | 机制已有，可读性没有 | **最小化**：每个 run 显示指纹、并显示它来自哪个快照；**不做**差异引擎 |

用一句话概括最小且诚实的 v0.5：**把第 5 步做实、把第 7 步做可读、把第 1–2 步的定义写下来。**

## 8. 只能由项目所有者拍板的决定

1. **导出范围**：按 run（§4 推荐）还是像今天这样整条链。
2. **导出落点**：工作区里一个可见路径、由另存为对话框选择（§4 推荐），还是固定的隐藏路径 `.riscdom/audit/`。
3. **区间从哪来**：导出命令接收 run id（§4 推荐），还是让 `RunView` 长出 `start_seq` / `end_seq`、由 UI 传。
4. **v0.5 是否要 CLI 导出**（推荐：不要），还是留给 v0.6。
5. **「环境」**：继续当作一个说法（§5 推荐），还是做成对象。
6. **发布门槛**：是否要求「每版由人在干净机器上用真实 API key 走通第 1–2 步」（推荐：要求，并写进 release
   notes），还是 `--ignored` e2e 测试就够了。
7. **第 7 步的最小化程度**：并排显示两个指纹（推荐），还是也显示**差在哪些字段** —— 后者是一个小差异引擎，
   会往 v0.6 漂。
8. **快照取名**：继续用 `window.prompt`（推荐：v0.5 不做，但它与旁边的原生对话框不一致），还是在 v0.5 顺手换掉。
