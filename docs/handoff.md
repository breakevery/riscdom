...
- **`v0.9.0` is the release** (2026-09-25): the version is bumped to `0.9.0` (7 files: `Cargo.toml`, the two
  ...
  that file verbatim** (the v0.7.0 and v0.8.0 releases worked that way). The release was cut the same day:
  annotated tag `v0.9.0` (object `fe4e0bc4411792a064945d8a5ec2c2e9add8bbe7` → `8bc7719`), a GitHub release
  whose body is that file verbatim, and **six assets** — the two Windows installers built on this machine
  plus the macOS `.dmg` and the Linux `.deb` / `.rpm` / `.AppImage` from the CI `bundle` job. The Latest
  marker moved to it. What v0.9 is:
...
- **Rust's sysroot is one click away now — and the one pin whose product is version-coupled**
  ...
  *before* any bytes move. **5 new tests** (the one-asset spec; both `find_rust_std` depths; the Rust
  end-to-end through a loopback server; adoption through `download_toolchain_now` asserting the C
  and Zig pins stay null; and the version refusal), plus the labels test extended to `rust` and a
  fixture builder that emits the GNU long-name entries the real archive has. Decision §52.
