...
- **帧只用 `id:` 与 `data:`，不设 `event:` 字段。** *决定与理由：* 设置 SSE 的 `event` 字段会让浏览器的 `EventSource` 派发到具名监听器、不再触发 `onmessage`，从而强迫客户端注册十四个监听器。不设则每帧都进一个 `onmessage` 处理器，由 envelope 的 `event` 字段负责路由。一个处理器，一个路由。
...
每帧都携带同一个对象。这是本批的核心产出：十四个事件各保留自己的 payload，但都装进一个 envelope，字段各有唯一含义。
...
| `event` | string \| null | 是 | 十四个名字之一；`hello` / `gap` 时为 `null`。 |
...
| 13 | `sandbox:switch`（v0.9 沙箱 F2b-2） | ——（v0.9 新增） | `{from, to, ok, reason}`——`from` 是切换前正在用的定义（没有则为 `null`），`to` 是被要求切到的那个，`ok` 为 `false` 时 `reason` 是原因码 | **新** |
| 14 | `sandbox:request`（v0.9 沙箱 F2c） | ——（v0.9 新增） | `{id, status, requester, action}`——每次变化一帧：申请落地时 `pending`，然后有人在决策时 `approved` / `rejected`；**批准不执行任何事** | **新** |

十四个中有**四个**改形状，**八个**是恒等映射，最后两个（`sandbox:switch`，F2b-2；`sandbox:request`，F2c）是 v0.9 新增——它们没有 v0.8 的 payload 可映射。
