...
- **帧只用 `id:` 与 `data:`，不设 `event:` 字段。** *决定与理由：* 设置 SSE 的 `event` 字段会让浏览器的 `EventSource` 派发到具名监听器、不再触发 `onmessage`，从而强迫客户端注册十三个监听器。不设则每帧都进一个 `onmessage` 处理器，由 envelope 的 `event` 字段负责路由。一个处理器，一个路由。
...
每帧都携带同一个对象。这是本批的核心产出：十三个事件各保留自己的 payload，但都装进一个 envelope，字段各有唯一含义。
...
| `event` | string \| null | 是 | 十三个名字之一；`hello` / `gap` 时为 `null`。 |
...
十三个中有**四个**改形状，**八个**是恒等映射，第十三个（`sandbox:switch`）是 v0.9 新增——它没有 v0.8 的 payload 可映射。
