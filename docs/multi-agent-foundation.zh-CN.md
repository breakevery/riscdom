...
6. **会话 DB 会等锁，但仍然没有 WAL**（v0.9）。SQLite 的 `busy_timeout` 默认是 0，第二个进程一写就当场失败——而打开失败会连带整个实例起不来。两个进程确实可能撞上同一个文件（两个走默认路径的 CLI / 服务器进程，或共用 `--data-dir`），因此连接现在会等 5 秒，且在它第一次写之前就设好，与审计库同一个形状。WAL **故意不开**，与审计库不同：那个库本来就要跨进程共享，这个是 per-instance，所以两者的并发模型本就不同（决策 §54）。
