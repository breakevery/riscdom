...
- **预览版的版本号在 Windows 里看起来不一样。** 若你装的是预览版 MSI，「设置 → 应用 → 已安装的应用」里
  显示的是 **`0.5.0.1`**，不是 `0.5.0-preview.1`：WiX 不接受预发布版本号，所以预览版单独钉了 MSI 自己的
  安装器版本号，而包版本 —— 也就是所有产物名 —— 仍是 `0.5.0-preview.1`。这是预期行为，**不是要报的 bug**；
  正式 `0.5.0` 会删掉这个覆盖（见 `CHANGELOG.zh-CN.md` 预览版条目的「备注」）。
- 导出的 run 记录**自足**：把文件放进一个空数据库，`audit-verify` 就能判定它，无需从产出它的机器上带任何东西
  （若还要 `--runs`，需先在该库重建派生索引，那是 `audit-rebuild` 的职责）。不能这样验证的文件是一个**发现**，
  不是格式细节。
