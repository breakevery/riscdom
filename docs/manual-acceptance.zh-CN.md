...
## 层次 9 —— 出：桌面端连到另一个节点（v0.9.9）

同机两个进程就够；第二台机器更好。两个节点**数据目录必须不同**——同目录就是同一个 `token` 文件，那样拿任何 token 都能过，等于什么都没验。

- [ ] 用**自己的数据目录**起第二个节点：`riscdom-server --bind 127.0.0.1:7822 --data-dir <dir-A> --workspace <dir-A/ws> --web-root ui/dist/app`（`ui/dist/app` 就是 `npm run build` 的产物）。注意端口是 **7822**——桌面端默认在 7821 服务自己的看板。
- [ ] 取那个节点的 token：`<dir-A>/token`。
- [ ] 在桌面端：*设置 → 网络*的**连出去**组 —— 填服务器地址（`http://127.0.0.1:7822`）与 token，点**连接**。页面会回服务器版本，或回连不上的原因。此时还没切换。
- [ ] **重启应用**（页面会这么说）。回来后**看的是另一个节点**：顶栏带一枚 *远程* 徽标并写明地址，状态页报的是**另一台**的 `agent_id`——这就是「屏幕上的数字不是本机的」的证据。
      **若重启后回到本机看板而不是登录门，请记为失败 —— 这就是 v0.9.9 4/N 那条未解释的观察（见 handoff）。**
- [ ] 设置里只提供 **Audit、Appearance、Network** 三个 tab：配置**本**节点的那些界面都不在了，因为这个窗口没在跟它说话。
- [ ] 停掉第二个节点。看板不崩：读取以一句话失败，窗口继续显示它上次读到的东西。
- [ ] **回本机**：在登录页（或 *设置 → 网络*）点**「断开并改用本机」**。地址被清空、钥匙串条目被删除、应用重启，本机节点自己的看板回来了。
- [ ] 在远端模式下关掉应用再打开：会自己回到那个远端节点（设置记得地址，钥匙串记得 token）。

## 通过标准

一次发布要走查成功，**层次 1–7 必须全过**。**层次 8 与 9 是最新的**，而两者这里都走不了：层次 8 需要一部手机，层次 9 需要第二个节点（同机、不同数据目录的第二个进程即可）。有条件时走一遍——缺了它们不算这次发布失败。
