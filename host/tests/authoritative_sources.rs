...
fn the_archive_predicate_accepts_the_agents_compiler_names() {
    for name in agent::GCC_NAMES {
        assert!(
            host::toolchain_download::is_compiler_name(Path::new(name)),
            "{name} must be recognised"
        );
        let with_exe = format!("{name}.exe");
        assert!(
            host::toolchain_download::is_compiler_name(Path::new(&with_exe)),
            "{with_exe} must be recognised"
        );
    }
    assert!(!host::toolchain_download::is_compiler_name(Path::new(
        "cmd.exe"
    )));
    assert!(!host::toolchain_download::is_compiler_name(Path::new(
